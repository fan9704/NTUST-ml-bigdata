...
### 1. L2 正則化（Ridge）與 L1 正則化（Lasso）最大的不同是？

- A L2 會完全忽略不重要的特徵
- B L2 不會讓權重歸零，而是將其盡量縮小 [X]
- C L2 較容易造成過擬合
- D L2 僅適用於決策樹模型

### 2. 交叉驗證中，最常見的 K-Fold Cross Validation 方法的主要目的為何？
...
### 3. 下列哪一項是 Boosting 技術的主要特點？

- A 對資料集進行有放回抽樣
- B 同時訓練多個模型，投票決策
- C 使用不同模型的預測結果當作新的標籤
- D 每一輪訓練更重視前一輪錯誤分類的樣本

### 4. XGBoost 在每輪建模時，是根據下列哪一項資訊來建立新樹的？

- A 上一輪模型的預測結果
- B 整體測試集準確率
- C 上一輪的預測殘差（residual） [X]
- D 權重最小的資料點

### 5. 在 XGBoost 中，若想避免模型過擬合，下列哪一個參數最直接與此有關？
...
### 1. 關於 Grid Search 的描述何者正確？

- A 完全依賴貝葉斯理論進行推測
- B 使用隨機方式搜尋超參數組合
- C 總是比 Random Search 更有效率
- D 枚舉所有參數組合進行訓練 [X]

### 2. 在神經網路中，「激活函數」的主要目的為何？
...
### 3. 神經網路的反向傳播（Backward Propagation）機制主要用來：
...
### 4. 卷積神經網路（CNN）中的池化層（Pooling Layer）主要功能為何？
...
### 1. 下列關於 Transformer 架構的描述，何者正確？

- A 必須逐步處理序列資料
- B 使用 RNN 結構進行語意建模
- C 核心為自注意力機制（Self-Attention）[X]
- D 僅能用於圖像分類任務


### 2. Autoencoder 最常用於哪一類型的任務？
...
### 3. 推薦系統的實作中，強化學習的「回報（Reward）」設計，下列哪一項通常屬於正向獎勵？
